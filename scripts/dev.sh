...
#   3. pontus-agent refuses to start without a token and refuses to serve a non-loopback
#      address without TLS. It installs software and removes data directories as root, so
#      both refusals are deliberate. The dev stack binds it to 127.0.0.1 and shares one
#      generated token between the agent and the backend's agent_token.
#   4. /go.sum is gitignored and the on-disk copy is incomplete, so a clean checkout
...
# A double-quoted scalar, at any indentation — agent_token sits under a backend.
cfg_scalar() {
  sed -n "s/^ *$1: *\"\([^\"]*\)\".*/\1/p" "$CONFIG" 2>/dev/null | head -1
}
...
reconcile_config() {
  ...
  if grep -q 'Generated by scripts/dev.sh' "$CONFIG" 2>/dev/null; then
    ...
    PROXY_PORT="$cp"
    MGMT_PORT="$mp"
    # Follow its agent too, or the agent listens somewhere the backend never dials.
    # An `if`, not `[ -n .. ] && ..`: that list returns 1 on an empty $ap, and it is the
    # last command in the function, so under `set -e` the caller would abort.
    if [ -n "$ap" ]; then
      AGENT_PORT="$ap"
    fi
  fi
...
backends:
  ...
  #
  # agent_token is mandatory too: the agent refuses to start without one, and the proxy
  # carries it on every RPC. Both ends must present the same value, so start_agent reads
  # it back out of this file rather than the two being generated independently.
  #
  # 127.0.0.1 is load-bearing. A cleartext agent is refused on both ends unless it is on
  # loopback, and this token authorises rebuilding a node as root.
...
EOF
  ...
  ok "wrote $CONFIG"
  info "${DIM}jwt_secret, admin_token and agent_token were generated randomly${N}"
}
...
start_agent() {
  ...
  # The agent exits rather than serving unauthenticated: its RPCs install software,
  # promote nodes and delete data directories as root. Take the token from the config so
  # the proxy presents the same one, and refuse to paper over a missing one with
  # -insecure — a dev stack that skips auth never exercises the path that ships.
  local token
  token="$(cfg_scalar agent_token)"
  [ -n "$token" ] || die "no agent_token in $CONFIG — add one to the backend, or pass --no-agent"

  # PONTUS_AGENT_TOKEN, not -token: argv is visible in a process listing.
  # 127.0.0.1, not :$AGENT_PORT — a wildcard bind is on the network, and the agent
  # refuses to serve one without TLS rather than let the token cross in cleartext.
  PONTUS_AGENT_TOKEN="$token" \
    "$BIN_DIR/pontus-agent" -addr "127.0.0.1:$AGENT_PORT" >"$LOG_DIR/agent.log" 2>&1 &
  track $!
